...
public class MosaicWriter_Raster implements MosaicWriter {
	...
	public static void write(Map<Integer,Patch> map, double llx, double lly, double cellsize, int nrows, int ncols, String species, String location) {
		double[][] data = new double[nrows][ncols];
		for (int i = 0; i < nrows; i++) {
			for (int j = 0; j < ncols; j++) {
				int key = i * ncols + j;
				if(map.get(key).hasNoData()){data[i][j] = -9999;}
				else{data[i][j] = map.get(key).isInfestedBy(species) ? 1 : 0;}
			}
		}
		RasterWriter rw = new RasterWriter();
		try {
			rw.setWriteHeader(true);
			rw.writeIntegerRaster(location, data, llx,
					lly, cellsize, "-9999");
		} catch (IOException e) {
			e.printStackTrace();
		}
